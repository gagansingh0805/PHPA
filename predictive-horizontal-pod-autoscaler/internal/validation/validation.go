...
func Validate(instance *phpav1alpha1.PredictiveHorizontalPodAutoscaler) error {
	spec := instance.Spec
...
func validateMinMax(spec phpav1alpha1.PredictiveHorizontalPodAutoscalerSpec) error {
...
func validateModels(models []phpav1alpha1.Model) error {
	for _, model := range models {
		if model.Type == phpav1alpha1.TypeHoltWinters {
			...
			if hw.RuntimeTuningFetchHook != nil {
				...
				if hook.Type == phpav1alpha1.HookTypeHTTP && hook.HTTP == nil {
		...
		if model.Type == phpav1alpha1.TypeLinear && model.Linear == nil {
