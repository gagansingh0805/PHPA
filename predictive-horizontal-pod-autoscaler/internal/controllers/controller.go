...
import (
	...
	"github.com/gagansingh0805/predictive-horizontal-pod-autoscaler/internal/scalebehavior"
...
const (
	defaultDecisionType = phpav1alpha1.DecisionMaximum
...
//+kubebuilder:rbac:groups=gagansingh.dev,resources=predictivehorizontalpodautoscalers,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=gagansingh.dev,resources=predictivehorizontalpodautoscalers/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=gagansingh.dev,resources=predictivehorizontalpodautoscalers/finalizers,verbs=update
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	phpaData := &phpav1alpha1.PredictiveHorizontalPodAutoscalerData{}
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			phpaData.ModelHistories = map[string]phpav1alpha1.ModelHistory{}
	...
	timestampedReplicaValue := phpav1alpha1.TimestampedReplicas{
	...
	if currentReplicas != targetReplicas {
		...
		if targetReplicas > currentReplicas {
			// Scale up
			scaleEvent := phpav1alpha1.TimestampedReplicas{
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) updateConfigMapData(ctx context.Context, configMap *corev1.ConfigMap,
	phpaData *phpav1alpha1.PredictiveHorizontalPodAutoscalerData) error {
	data, err := json.Marshal(phpaData)
	if err != nil {
		// Should not occur, panic
		panic(err)
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) processModels(ctx context.Context,
	instance *phpav1alpha1.PredictiveHorizontalPodAutoscaler,
	phpaData *phpav1alpha1.PredictiveHorizontalPodAutoscalerData, now time.Time, currentReplicas int32,
	calculatedReplicas int32) ([]int32, *phpav1alpha1.PredictiveHorizontalPodAutoscalerData) {
	...
	for _, model := range instance.Spec.Models {
		...
		if !exists || modelHistory.Type != model.Type {
			...
			modelHistory = phpav1alpha1.ModelHistory{
				Type:              model.Type,
				SyncPeriodsPassed: 1,
		...
		if model.ResetDuration != nil && len(modelHistory.ReplicaHistory) > 0 {
			...
			if durationSinceLastData > model.ResetDuration.Duration {
				...
				modelHistory.ReplicaHistory = []phpav1alpha1.TimestampedReplicas{}
		...
		modelHistory.ReplicaHistory = append(modelHistory.ReplicaHistory, phpav1alpha1.TimestampedReplicas{
			Time: &metav1.Time{
				Time: now,
			},
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) calculateReplicas(
	instance *phpav1alpha1.PredictiveHorizontalPodAutoscaler, scale *autoscalingv1.Scale) (int32, error) {
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) preScaleStatusCheck(ctx context.Context,
	instance *phpav1alpha1.PredictiveHorizontalPodAutoscaler) error {
...
func (r *PredictiveHorizontalPodAutoscalerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&phpav1alpha1.PredictiveHorizontalPodAutoscaler{}).
